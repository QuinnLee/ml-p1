...
if __name__ == '__main__':
    ...
    get_abspath('abalone.csv', 'data')
    opts_digit_df = pd.read_csv('../data/optdigits_train.csv', header=None)
    opts_digit_df.rename({64: 'class'}, axis='columns', inplace=True)

    dfs = {
        'contraceptive': contraceptive_df,
        'abalone': abalone_df,
        'opts_digit': opts_digit_df,
    }

    if args.dataset:
        dnames = [args.dataset]
    else:
        dnames = ['opts_digit', 'abalone']
    ...
    mnames = ['DT', 'SVM_RBF', 'SVM_PLY', 'Boosting', 'ANN', 'KNN']
    ...
    for df in dnames:
        ...
        for name, estimator in estimators.iteritems():
            ...
            create_timing_curve(
                    estimator.best_estimator_,
                    dataset=dfs[df].copy(deep=True),
                    data_name=df,
                    clf_name=name
            )
            create_validation_curve(
                    estimator.best_estimator_,
                    X_train, y_train, data_name=df,
                    clf_name=name,
